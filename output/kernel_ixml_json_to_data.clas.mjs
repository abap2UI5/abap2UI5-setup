...
class kernel_ixml_json_to_data {
  ...
  static async get_field_name(INPUT) {
    ...
    if (abap.compare.initial(attr) === false) {
      ...
      let unique686 = 1;
      while (true) {
        abap.builtin.sy.get().index.set(unique686++);
  ...
  static async build(INPUT) {
    ...
    let unique687 = 1;
    while (true) {
      abap.builtin.sy.get().index.set(unique687++);
  ...
  static async traverse(INPUT) {
    ...
    if (abap.compare.eq(unique688, abap.Classes['CL_ABAP_TYPEDESCR'].kind_struct)) {
      ...
      let unique689 = 1;
      while (true) {
        abap.builtin.sy.get().index.set(unique689++);
      ...
      let unique690 = 1;
      while (true) {
        abap.builtin.sy.get().index.set(unique690++);
        li_child.set((await li_iterator.get().if_ixml_node_iterator$get_next()));
        if (abap.compare.initial(li_child)) {
          break;
        }
