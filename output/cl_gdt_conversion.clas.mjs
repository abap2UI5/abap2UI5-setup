...
class cl_gdt_conversion {
  ...
  static async amount_outbound(INPUT) {
    ...
    let unique219 = im_currency_code;
    if (abap.compare.eq(unique219, new abap.types.Character(3).set('DKK')) || abap.compare.eq(unique219, new abap.types.Character(3).set('EUR')) || abap.compare.eq(unique219, new abap.types.Character(3).set('USD'))) {
      ex_value.set(im_value);
    } else if (abap.compare.eq(unique219, new abap.types.Character(3).set('VND'))) {
  ...
  static async unit_code_outbound(INPUT) {
    ...
    let unique220 = im_value;
    if (abap.compare.eq(unique220, new abap.types.Character(1).set(''))) {
      const unique221 = await (new abap.Classes['CX_GDT_CONVERSION']()).constructor_();
      unique221.EXTRA_CX = {"INTERNAL_FILENAME": "cl_gdt_conversion.clas.abap","INTERNAL_LINE": 81};
      throw unique221;
    } else if (abap.compare.eq(unique220, new abap.types.Character(2).set('ST'))) {
      ex_value.set(new abap.types.Character(3).set('PCE'));
    } else if (abap.compare.eq(unique220, new abap.types.Character(2).set('KG'))) {
      ex_value.set(new abap.types.Character(3).set('KGM'));
    } else if (abap.compare.eq(unique220, new abap.types.Character(3).set('CDM'))) {
  ...
  static async unit_code_inbound(INPUT) {
    ...
    let unique222 = im_value;
    if (abap.compare.eq(unique222, new abap.types.Character(3).set('MTR'))) {
      ex_value.set(new abap.types.Character(1).set('M'));
    } else if (abap.compare.eq(unique222, new abap.types.Character(3).set('PCE'))) {
      ex_value.set(new abap.types.Character(2).set('PC'));
    } else if (abap.compare.eq(unique222, new abap.types.Character(3).set('KGM'))) {
      ex_value.set(new abap.types.Character(2).set('KG'));
    } else if (abap.compare.eq(unique222, new abap.types.Character(3).set('LTR'))) {
  ...
  static async language_code_outbound(INPUT) {
    ...
    try {
      ...
      if (abap.compare.ne(abap.builtin.sy.get().subrc, abap.IntegerFactory.get(0))) {
        const unique223 = await (new abap.Classes['CX_GDT_CONVERSION']()).constructor_();
        unique223.EXTRA_CX = {"INTERNAL_FILENAME": "cl_gdt_conversion.clas.abap","INTERNAL_LINE": 143};
        throw unique223;
      }
    }
  }
