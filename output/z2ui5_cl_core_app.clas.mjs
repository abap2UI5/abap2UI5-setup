...
class z2ui5_cl_core_app {
  ...
  async all_xml_stringify() {
    ...
    try {
      ...
      if ((abap.Classes['CX_ROOT'] && e instanceof abap.Classes['CX_ROOT'])) {
        ...
        unique121.EXTRA_CX = {"INTERNAL_FILENAME": "z2ui5_cl_core_app.clas.abap","INTERNAL_LINE": 85};
        throw unique121;
      } else {
        throw e;
      }
  ...
  static async db_load(INPUT) {
    ...
    temp2.assign(result.get().mt_attri);
    lo_model.set(await (new abap.Classes['Z2UI5_CL_CORE_ATTRI_SRV']()).constructor_({attri: temp2, app: result.get().mo_app}));
  ...
  static async db_load_by_app(INPUT) {
    ...
    temp3.assign(result.get().mt_attri);
    lo_model.set(await (new abap.Classes['Z2UI5_CL_CORE_ATTRI_SRV']()).constructor_({attri: temp3, app: result.get().mo_app}));
    await lo_model.get().attri_refs_update();
    return result;
  }
  async db_save() {
    let temp4 = new abap.types.ABAPObject({qualifiedName: "Z2UI5_IF_APP", RTTIName: "\\INTERFACE=Z2UI5_IF_APP"});
    let lo_db = new abap.types.ABAPObject({qualifiedName: "Z2UI5_CL_CORE_DRAFT_SRV", RTTIName: "\\CLASS=Z2UI5_CL_CORE_DRAFT_SRV"});
    if (abap.compare.initial(this.mo_app) === false) {
      await abap.statements.cast(temp4, this.mo_app);
      temp4.get().z2ui5_if_app$id_draft.set(this.ms_draft.get().id);
    }
    lo_db.set(await (new abap.Classes['Z2UI5_CL_CORE_DRAFT_SRV']()).constructor_());
  ...
  async model_json_parse(INPUT) {
    ...
    temp5.assign(this.mt_attri);
    await lo_json_mapper.get().model_front_to_back({view: iv_view, t_attri: temp5, model: io_model});
  }
