...
class kernel_scan_abap_source {
  ...
  static async call(INPUT) {
    ...
    for await (const unique156 of abap.statements.loop(lt_stokesx)) {
      ls_stokesx.set(unique156);
  ...
  static async pass2(INPUT) {
    ...
    for await (const unique158 of abap.statements.loop(ct_statements)) {
      fs_ls_statement_.assign(unique158);
      ...
      for await (const unique159 of abap.statements.loop(ct_tokens,{from: fs_ls_statement_.get().from,to: fs_ls_statement_.get().to})) {
        ls_token.set(unique159);
      ...
      if (abap.compare.eq(contains_comment, abap.builtin.abap_true) && abap.compare.eq(contains_normal, abap.builtin.abap_true)) {
        ...
        for await (const unique161 of abap.statements.loop(lt_delete)) {
          lv_index.set(unique161);
          await abap.statements.deleteInternal(ct_tokens,{index: lv_index});
        }
        for await (const unique162 of abap.statements.loop(lt_insert)) {
          ls_token.set(unique162);
