...
class kernel_unit_runner {
  ...
  static async get_location(INPUT) {
    ...
    for await (const unique778 of abap.statements.loop(lt_lines)) {
      lv_stack.set(unique778);
  ...
  static async to_json(INPUT) {
    ...
    for await (const unique779 of abap.statements.loop(it_list)) {
      ls_list.set(unique779);
  ...
  static async unique_classes(INPUT) {
    ...
    for await (const unique780 of abap.statements.loop(it_input)) {
      ls_input.set(unique780);
  ...
  static async run(INPUT) {
    ...
    for await (const unique781 of abap.statements.loop(lt_classes)) {
      ls_class.set(unique781);
      ...
      let unique782 = abap.Classes["CLAS-KERNEL_UNIT_RUNNER-"+lv_name.get().trimEnd()];
      if (unique782 === undefined) { unique782 = abap.Classes[lv_name.get().trimEnd()]; }
      if (unique782 === undefined) { throw new abap.Classes['CX_SY_CREATE_OBJECT_ERROR']; }
      lo_obj.set(await (new unique782()).constructor_());
      try {
      ...
      for await (const unique783 of abap.statements.loop(it_input,{where: async (I) => {return abap.compare.eq(I.class_name, ls_class.get().class_name) && abap.compare.eq(I.testclass_name, ls_class.get().testclass_name);},topEquals: {"class_name": ls_class.get().class_name,"testclass_name": ls_class.get().testclass_name}})) {
        ls_input.set(unique783);
