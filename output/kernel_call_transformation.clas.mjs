  ...
  static async call(INPUT) {
    ...
    if (abap.compare.initial(lv_source) === false) {
      if (abap.compare.eq(lv_source.getOffset({length: 1}), new abap.types.Character(1).set('<'))) {
        ...
        const unique268 = await (new abap.Classes['CX_XSLT_FORMAT_ERROR']()).constructor_();
        unique268.EXTRA_CX = {"INTERNAL_FILENAME": "kernel_call_transformation.clas.abap","INTERNAL_LINE": 77};
        throw unique268;
      }
    }
          ...
          if (INPUT.source.constructor.name === "Object") {
              for (const name in INPUT.source) {
                ...
                if (abap.compare.initial(lv_source) && abap.compare.initial(kernel_call_transformation.mi_doc)) {
                  const unique269 = await (new abap.Classes['CX_XSLT_RUNTIME_ERROR']()).constructor_();
                  unique269.EXTRA_CX = {"INTERNAL_FILENAME": "kernel_call_transformation.clas.abap","INTERNAL_LINE": 147};
                  throw unique269;
                }
                if (INPUT.result.constructor.name === "Table") {
                  lt_rtab = INPUT.result;
                  for await (const unique270 of abap.statements.loop(lt_rtab)) {
                    ls_rtab.set(unique270);
                  ...
                  static async traverse_write_type(INPUT) {
                    ...
                    let unique271 = lo_type.get().type_kind;
                    if (abap.compare.eq(unique271, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_int) || abap.compare.eq(unique271, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_int1) || abap.compare.eq(unique271, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_int2) || abap.compare.eq(unique271, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_int8) || abap.compare.eq(unique271, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_decfloat) || abap.compare.eq(unique271, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_decfloat16) || abap.compare.eq(unique271, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_decfloat34)) {
                  ...
                  static async traverse_write(INPUT) {
                    ...
                    lo_type.set((await abap.Classes['CL_ABAP_TYPEDESCR'].describe_by_data({p_data: (iv_ref).dereference()})));
                    let unique272 = lo_type.get().kind;
                    if (abap.compare.eq(unique272, abap.Classes['CL_ABAP_TYPEDESCR'].kind_struct)) {
                      await kernel_call_transformation.mi_writer.get().if_sxml_writer$open_element({name: new abap.types.Character(6).set('object')});
                      ...
                      for await (const unique273 of abap.statements.loop(lt_comps)) {
                        ls_compo.set(unique273);
                      ...
                      await kernel_call_transformation.mi_writer.get().if_sxml_writer$close_element();
                    } else if (abap.compare.eq(unique272, abap.Classes['CL_ABAP_TYPEDESCR'].kind_elem)) {
                      await kernel_call_transformation.mi_writer.get().if_sxml_writer$write_value({value: (iv_ref).dereference()});
                    } else if (abap.compare.eq(unique272, abap.Classes['CL_ABAP_TYPEDESCR'].kind_table)) {
                      ...
                      for await (const unique274 of abap.statements.loop(fs_table_)) {
                        fs_any_.assign(unique274);
