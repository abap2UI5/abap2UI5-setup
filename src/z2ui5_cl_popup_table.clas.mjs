...
class z2ui5_cl_popup_table {
  ...
  async display() {
    ...
    for await (const unique184 of abap.statements.loop(lt_comp)) {
      ls_comp.set(unique184);
  ...
  async on_event() {
    let unique186 = ((await this.client.get().z2ui5_if_client$get())).get().event;
    if (abap.compare.eq(unique186, new abap.types.Character(14).set('BUTTON_CONFIRM'))) {
      this.ms_result.get().check_confirmed.set(abap.builtin.abap_true);
      await this.on_event_confirm();
    } else if (abap.compare.eq(unique186, new abap.types.Character(6).set('CANCEL'))) {
