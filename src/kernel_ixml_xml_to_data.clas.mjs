...
class kernel_ixml_xml_to_data {
  ...
  static async traverse(INPUT) {
    ...
    let unique281 = lo_type.get().kind;
    if (abap.compare.eq(unique281, abap.Classes['CL_ABAP_TYPEDESCR'].kind_struct)) {
      ...
      abap.builtin.sy.get().index.set(indexBackup1);
    } else if (abap.compare.eq(unique281, abap.Classes['CL_ABAP_TYPEDESCR'].kind_elem)) {
    ...
    } else if (abap.compare.eq(unique281, abap.Classes['CL_ABAP_TYPEDESCR'].kind_table)) {
    ...
    } else if (abap.compare.eq(unique281, abap.Classes['CL_ABAP_TYPEDESCR'].kind_ref)) {
      abap.statements.assign({target: fs_any_, source: (iv_ref).dereference()});
