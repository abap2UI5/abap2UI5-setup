...
class cl_express_icf_shim {
  ...
  static async run(INPUT) {
    ...
    let unique788 = abap.Classes["CLAS-CL_EXPRESS_ICF_SHIM-"+lv_classname.get().trimEnd()];
    if (unique788 === undefined) { unique788 = abap.Classes[lv_classname.get().trimEnd()]; }
    if (unique788 === undefined) { throw new abap.Classes['CX_SY_CREATE_OBJECT_ERROR']; }
    li_handler.set(await (new unique788()).constructor_());
  ...
  static async response(INPUT) {
    ...
    for await (const unique789 of abap.statements.loop(lt_header_fields)) {
      ls_field.set(unique789);
