...
class cl_http_client {
  ...
  async if_http_client$send(INPUT) {
    ...
    if (abap.compare.gt(abap.builtin.lines({val: lt_form_fields}), abap.IntegerFactory.get(0))) {
      let unique636 = lv_method;
      if (abap.compare.eq(unique636, new abap.types.Character(3).set('GET'))) {
      ...
      } else if (abap.compare.eq(unique636, new abap.types.Character(4).set('POST'))) {
    ...
    let headers = {};
    for await (const unique637 of abap.statements.loop(lt_header_fields,{where: async (I) => {return abap.compare.ne(I.name, new abap.types.Character(12).set('~request_uri'));}})) {
      ls_field.set(unique637);
