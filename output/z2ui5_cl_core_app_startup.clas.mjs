...
class z2ui5_cl_core_app_startup {
  ...
  "MT_CLASSES": {"type": () => {return abap.types.TableFactory.construct(new abap.types.String({qualifiedName: "STRING"}), {"withHeader":false,"keyType":"DEFAULT","primaryKey":{"isUnique":false,"type":"STANDARD","keyFields":[],"name":"primary_key"},"secondary":[]}, "STRING_TABLE");}, "visibility": "O", "is_constant": " ", "is_class": " "},
  ...
  constructor() {
    ...
    this.mt_classes = abap.types.TableFactory.construct(new abap.types.String({qualifiedName: "STRING"}), {"withHeader":false,"keyType":"DEFAULT","primaryKey":{"isUnique":false,"type":"STANDARD","keyFields":[],"name":"primary_key"},"secondary":[]}, "STRING_TABLE");
  ...
  async on_event_check() {
    ...
    try {
      ...
      let unique134 = abap.Classes["CLAS-Z2UI5_CL_CORE_APP_STARTUP-"+this.ms_home.get().classname.get().trimEnd()];
      if (unique134 === undefined) { unique134 = abap.Classes[this.ms_home.get().classname.get().trimEnd()]; }
      if (unique134 === undefined) { throw new abap.Classes['CX_SY_CREATE_OBJECT_ERROR']; }
      li_app_test.set(await (new unique134()).constructor_());
      ...
      this.ms_home.get().url.set((await abap.Classes['Z2UI5_CL_UTIL'].app_get_url({client: this.client, classname: this.ms_home.get().classname})));
    } catch (e) {
  ...
  async view_display_start() {
    ...
    let lv_url_samples3 = new abap.types.String({qualifiedName: "STRING"});
    ...
    page2.set((await (await (await abap.Classes['Z2UI5_CL_XML_VIEW'].factory()).get().shell()).get().page({shownavbutton: abap.builtin.abap_false})));
    await (await (await (await (await (await page2.get().header_content()).get().text()).get().title({text: new abap.types.String().set(`abap2UI5 - Developing UI5 Apps Purely in ABAP`)})).get().toolbar_spacer()).get().button({text: new abap.types.String().set(`Debugging Tools`), icon: new abap.types.String().set(`sap-icon://enablement`), press: (await this.client.get().z2ui5_if_client$_event({val: new abap.types.String().set(`OPEN_DEBUG`)}))})).get().button({text: new abap.types.String().set(`System`), icon: new abap.types.String().set(`sap-icon://information`), press: (await this.client.get().z2ui5_if_client$_event({val: new abap.types.String().set(`OPEN_INFO`)}))});
    ...
    lv_url_samples2.set((await abap.Classes['Z2UI5_CL_UTIL'].app_get_url({client: this.client, classname: new abap.types.Character(21).set('z2ui5_cl_demo_app_000')})));
    lv_url_samples3.set((await abap.Classes['Z2UI5_CL_UTIL'].app_get_url({client: this.client, classname: new abap.types.Character(24).set('z2ui5_cl_core_app_search')})));
    await (await simple_form2.get().toolbar()).get().title({text: new abap.types.String().set(`What's next?`)});
    await simple_form2.get().label({text: new abap.types.String().set(`App Finder`)});
    abap.statements.clear(temp1);
    abap.statements.insertInternal({data: lv_url_samples3, table: temp1});
    await simple_form2.get().button({text: new abap.types.String().set(`Start & Install Apps`), press: (await this.client.get().z2ui5_if_client$_event_client({val: this.client.get().z2ui5_if_client$cs_event.get().open_new_tab, t_arg: temp1})), width: new abap.types.String().set(`70%`)});
    if (abap.compare.initial((await abap.Classes['Z2UI5_CL_UTIL'].rtti_check_class_exists({val: new abap.types.String().set(`z2ui5_cl_demo_app_000`)}))) === false) {
      ...
      abap.statements.insertInternal({data: lv_url_samples2, table: temp3});
      await simple_form2.get().button({text: new abap.types.String().set(`Explore Code Samples`), press: (await this.client.get().z2ui5_if_client$_event_client({val: this.client.get().z2ui5_if_client$cs_event.get().open_new_tab, t_arg: temp3})), width: new abap.types.String().set(`70%`)});
    } else {
    ...
    await simple_form2.get().label({text: new abap.types.String().set(``)});
    await simple_form2.get().text({text: new abap.types.String().set(``)});
    ...
    await simple_form2.get().label({text: new abap.types.String().set(``)});
    await simple_form2.get().text({text: new abap.types.String().set(``)});
    await simple_form2.get().label({text: new abap.types.String().set(`Links & More`)});
    await simple_form2.get().link({text: new abap.types.String().set(`Repository on GitHub`), target: new abap.types.String().set(`_blank`), href: new abap.types.String().set(`https://github.com/abap2UI5/abap2UI5`)});
    await simple_form2.get().label();
    await simple_form2.get().link({text: new abap.types.String().set(`News on Twitter`), target: new abap.types.String().set(`_blank`), href: new abap.types.String().set(`https://twitter.com/abap2UI5`)});
    await simple_form2.get().label();
    await simple_form2.get().link({text: new abap.types.String().set(`Blog Series on SAP Community`), target: new abap.types.String().set(`_blank`), href: new abap.types.String().set(`https://community.sap.com/t5/technology-blogs-by-members/abap2ui5-1-introduction-developing-ui5-apps-purely-in-abap/ba-p/13567635`)});
  ...
  async z2ui5_if_app$main(INPUT) {
    ...
    let temp5 = new abap.types.ABAPObject({qualifiedName: "Z2UI5_CL_POPUP_TO_SELECT", RTTIName: "\\CLASS=Z2UI5_CL_POPUP_TO_SELECT"});
    let lo_f4 = new abap.types.ABAPObject({qualifiedName: "Z2UI5_CL_POPUP_TO_SELECT", RTTIName: "\\CLASS=Z2UI5_CL_POPUP_TO_SELECT"});
    let ls_result = new abap.types.Structure({"row": new abap.types.DataReference(new abap.types.Character(4)), "check_confirmed": new abap.types.Character(1, {"qualifiedName":"ABAP_BOOL","ddicName":"ABAP_BOOL"})}, "z2ui5_cl_popup_to_select=>ty_s_result", undefined, {}, {});
    ...
    if (abap.compare.eq(((await client.get().z2ui5_if_client$get())).get().check_on_navigated, abap.builtin.abap_true)) {
      try {
        await abap.statements.cast(temp5, (await client.get().z2ui5_if_client$get_app({id: ((await client.get().z2ui5_if_client$get())).get().s_draft.get().id_prev_app})));
        lo_f4.set(temp5);
        ...
        if (abap.compare.eq(ls_result.get().check_confirmed, abap.builtin.abap_true)) {
          ...
          this.ms_home.get().classname.set(fs_class_);
          await this.view_display_start();
          return;
        }
      } catch (e) {
  ...
  async z2ui5_on_event() {
    let unique135 = ((await this.client.get().z2ui5_if_client$get())).get().event;
    if (abap.compare.eq(unique135, new abap.types.String().set(`OPEN_DEBUG`))) {
    ...
    } else if (abap.compare.eq(unique135, new abap.types.String().set(`OPEN_INFO`))) {
      await this.client.get().z2ui5_if_client$nav_app_call({app: (await abap.Classes['Z2UI5_CL_CORE_APP_INFO'].factory())});
      return;
    } else if (abap.compare.eq(unique135, new abap.types.String().set(`BUTTON_CHECK`))) {
    ...
    } else if (abap.compare.eq(unique135, new abap.types.Character(10).set('VALUE_HELP'))) {
      try {
        this.mt_classes.set((await abap.Classes['Z2UI5_CL_UTIL'].rtti_get_classes_impl_intf({val: new abap.types.String().set(`Z2UI5_IF_APP`)})));
        await this.client.get().z2ui5_if_client$nav_app_call({app: (await abap.Classes['Z2UI5_CL_POPUP_TO_SELECT'].factory({i_tab: this.mt_classes}))});
      } catch (e) {
        if ((abap.Classes['CX_ROOT'] && e instanceof abap.Classes['CX_ROOT'])) {
          await this.client.get().z2ui5_if_client$message_box_display({text: new abap.types.String().set(`The value help is not available on your system, upgrade to a higher release first`)});
        } else {
          throw e;
        }
      }
  ...
  async z2ui5_on_init() {
    ...
    this.ms_home.get().class_editable.set(abap.builtin.abap_true);
    this.ms_home.get().btn_icon.set(new abap.types.String().set(`sap-icon://validate`));
    this.ms_home.get().classname.set(new abap.types.String().set(`Z2UI5_CL_APP_HELLO_WORLD`));
  }
}
...
z2ui5_cl_core_app_startup.z2ui5_if_app$version.set('1.120.0');
