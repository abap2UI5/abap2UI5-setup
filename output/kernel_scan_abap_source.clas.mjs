...
class kernel_scan_abap_source {
  ...
  static async call(INPUT) {
    ...
    for await (const unique161 of abap.statements.loop(lt_stokesx)) {
      ls_stokesx.set(unique161);
  ...
  static async pass1(INPUT) {
    ...
    while (abap.compare.initial(source) === false) {
      abap.builtin.sy.get().index.set(unique162++);
  ...
  static async pass2(INPUT) {
    ...
    for await (const unique163 of abap.statements.loop(ct_statements)) {
      fs_ls_statement_.assign(unique163);
      ...
      for await (const unique164 of abap.statements.loop(ct_tokens,{from: fs_ls_statement_.get().from,to: fs_ls_statement_.get().to})) {
        ls_token.set(unique164);
      ...
      if (abap.compare.eq(contains_comment, abap.builtin.abap_true) && abap.compare.eq(contains_normal, abap.builtin.abap_true)) {
        ...
        for await (const unique165 of abap.statements.loop(ct_tokens,{from: fs_ls_statement_.get().from,to: fs_ls_statement_.get().to})) {
          ls_token.set(unique165);
          if (abap.compare.eq(ls_token.get().type, kernel_scan_abap_source.gc_token.get().comment)) {
        ...
        for await (const unique166 of abap.statements.loop(lt_delete)) {
          lv_index.set(unique166);
          await abap.statements.deleteInternal(ct_tokens,{index: lv_index});
        }
        for await (const unique167 of abap.statements.loop(lt_insert)) {
          ls_token.set(unique167);
