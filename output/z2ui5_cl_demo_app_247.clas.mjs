...
// z2ui5_cl_demo_app_247.clas.abap
class z2ui5_cl_demo_app_247 {
  static INTERNAL_TYPE = 'CLAS';
  static INTERNAL_NAME = 'Z2UI5_CL_DEMO_APP_247';
  ...
  async display_view(INPUT) {
    ...
    page.set((await (await (await abap.Classes['Z2UI5_CL_XML_VIEW'].factory()).get().shell()).get().page({title: new abap.types.Character(44).set('abap2UI5 - Sample: Splitter Layout - 2 areas'), navbuttonpress: (await client.get().z2ui5_if_client$_event({val: new abap.types.Character(4).set('BACK')})), shownavbutton: temp1})));
    ...
    await (await page.get().header_content()).get().link({text: new abap.types.Character(12).set('UI5 Demo Kit'), target: new abap.types.Character(6).set('_blank'), href: new abap.types.Character(106).set('https://sapui5.hana.ondemand.com/sdk/#/entity/sap.ui.layout.Splitter/sample/sap.ui.layout.sample.Splitter2')});
    layout.set((await (await (await (await (await (await (await (await (await (await (await (await page.get().splitter({height: new abap.types.String().set(`500px`)})).get().button({width: new abap.types.String().set(`100%`), text: new abap.types.String().set(`Content 1`)})).get().get()).get().layout_data()).get().splitter_layout_data({size: new abap.types.String().set(`300px`)})).get().get_parent()).get().get_parent()).get().get_parent()).get().button({width: new abap.types.String().set(`100%`), text: new abap.types.String().set(`Content 2`)})).get().get()).get().layout_data()).get().splitter_layout_data({size: new abap.types.String().set(`auto`)})));
  ...
  async on_event(INPUT) {
    ...
    if (client?.getQualifiedName === undefined || client.getQualifiedName() !== "Z2UI5_IF_CLIENT") { client = undefined; }
    if (client === undefined) { client = new abap.types.ABAPObject({qualifiedName: "Z2UI5_IF_CLIENT", RTTIName: "\\INTERFACE=Z2UI5_IF_CLIENT"}).set(INPUT.client); }
    let unique542 = ((await client.get().z2ui5_if_client$get())).get().event;
    if (abap.compare.eq(unique542, new abap.types.Character(4).set('BACK'))) {
      await client.get().z2ui5_if_client$nav_app_leave();
    } else if (abap.compare.eq(unique542, new abap.types.Character(7).set('POPOVER'))) {
...
abap.Classes['Z2UI5_CL_DEMO_APP_247'] = z2ui5_cl_demo_app_247;
z2ui5_cl_demo_app_247.z2ui5_if_app$version = new abap.types.String({qualifiedName: "STRING"});
z2ui5_cl_demo_app_247.z2ui5_if_app$version.set('1.135.0');
z2ui5_cl_demo_app_247.z2ui5_if_app$origin = new abap.types.String({qualifiedName: "STRING"});
z2ui5_cl_demo_app_247.z2ui5_if_app$origin.set('https://github.com/abap2UI5/abap2UI5');
z2ui5_cl_demo_app_247.z2ui5_if_app$author = new abap.types.String({qualifiedName: "STRING"});
z2ui5_cl_demo_app_247.z2ui5_if_app$author.set('https://github.com/oblomov-dev');
z2ui5_cl_demo_app_247.z2ui5_if_app$license = new abap.types.String({qualifiedName: "STRING"});
z2ui5_cl_demo_app_247.z2ui5_if_app$license.set('MIT');
export {z2ui5_cl_demo_app_247};
//# sourceMappingURL=z2ui5_cl_demo_app_247.clas.mjs.map