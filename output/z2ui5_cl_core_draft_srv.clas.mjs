...
class z2ui5_cl_core_draft_srv {
  ...
  async cleanup() {
    ...
    await abap.statements.deleteDatabase("z2ui5_t_core_01", {"where": "\"timestampl\" < '" + lv_four_hours_ago.get() + "'"});
  ...
  async create(INPUT) {
    ...
    temp1.get().uname.set((await abap.Classes['Z2UI5_CL_UTIL'].user_get_tech()));
    temp1.get().timestampl.set((await abap.Classes['Z2UI5_CL_UTIL'].time_get_timestampl()));
    temp1.get().data.set(model_xml);
    ls_db.set(temp1);
    await abap.statements.modifyDatabase("z2ui5_t_core_01", {"values": ls_db});
    if (abap.compare.ne(abap.builtin.sy.get().subrc, abap.IntegerFactory.get(0))) {
      const unique110 = await (new abap.Classes['Z2UI5_CX_UTIL_ERROR']()).constructor_({val: new abap.types.String().set(`CREATE_OF_DRAFT_ENTRY_ON_DATABASE_FAILED`)});
      unique110.EXTRA_CX = {"INTERNAL_FILENAME": "z2ui5_cl_core_draft_srv.clas.abap","INTERNAL_LINE": 81};
      throw unique110;
    }
    abap.statements.commit();
  }
  async read(INPUT) {
    ...
    if (abap.compare.eq(check_load_app, abap.builtin.abap_true)) {
      await abap.statements.select(result, {select: "SELECT * FROM " + abap.buildDbTableName("z2ui5_t_core_01") + " WHERE \"id\" = '" + id.get() + "' UP TO 1 ROWS", primaryKey: ["mandt","id"]});
    } else {
      await abap.statements.select(result, {select: "SELECT \"id\", \"id_prev\", \"id_prev_app\", \"id_prev_app_stack\" FROM " + abap.buildDbTableName("z2ui5_t_core_01") + " WHERE \"id\" = '" + id.get() + "' UP TO 1 ROWS", primaryKey: ["mandt","id"]});
    }
    if (abap.compare.ne(abap.builtin.sy.get().subrc, abap.IntegerFactory.get(0))) {
      const unique111 = await (new abap.Classes['Z2UI5_CX_UTIL_ERROR']()).constructor_({val: new abap.types.String().set(`NO_DRAFT_ENTRY_OF_PREVIOUS_REQUEST_FOUND`)});
      unique111.EXTRA_CX = {"INTERNAL_FILENAME": "z2ui5_cl_core_draft_srv.clas.abap","INTERNAL_LINE": 109};
      throw unique111;
    }
    return result;
  }
  ...
  async count_entries() {
    ...
    await abap.statements.select(result, {select: "SELECT COUNT( * )  FROM " + abap.buildDbTableName("z2ui5_t_core_01") + "", primaryKey: ["mandt","id"]});
    return result;
  }
}
