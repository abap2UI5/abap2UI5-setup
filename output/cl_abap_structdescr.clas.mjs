...
class cl_abap_structdescr extends cl_abap_complexdescr {
  ...
  static async create(INPUT) {
    ...
    if (abap.compare.eq(abap.builtin.lines({val: p_components}), abap.IntegerFactory.get(0))) {
      const unique718 = await (new abap.Classes['CX_SY_STRUCT_ATTRIBUTES']()).constructor_();
      unique718.EXTRA_CX = {"INTERNAL_FILENAME": "cl_abap_structdescr.clas.abap","INTERNAL_LINE": 87};
      throw unique718;
    }
    for await (const unique719 of abap.statements.loop(p_components)) {
      ls_component.set(unique719);
      if (abap.compare.initial(ls_component.get().name)) {
        const unique720 = await (new abap.Classes['CX_SY_STRUCT_COMP_NAME']()).constructor_();
        unique720.EXTRA_CX = {"INTERNAL_FILENAME": "cl_abap_structdescr.clas.abap","INTERNAL_LINE": 92};
        throw unique720;
      } else if (abap.compare.initial(ls_component.get().type)) {
        const unique721 = await (new abap.Classes['CX_SY_STRUCT_COMP_TYPE']()).constructor_();
        unique721.EXTRA_CX = {"INTERNAL_FILENAME": "cl_abap_structdescr.clas.abap","INTERNAL_LINE": 94};
        throw unique721;
      } else if (abap.compare.gt(abap.builtin.strlen({val: ls_component.get().name}), abap.IntegerFactory.get(30))) {
        const unique722 = await (new abap.Classes['CX_SY_STRUCT_COMP_NAME']()).constructor_();
        unique722.EXTRA_CX = {"INTERNAL_FILENAME": "cl_abap_structdescr.clas.abap","INTERNAL_LINE": 96};
        throw unique722;
      }
    }
    ref.set(await (new abap.Classes['CL_ABAP_STRUCTDESCR']()).constructor_());
    for await (const unique723 of abap.statements.loop(p_components)) {
      ls_component.set(unique723);
  ...
  async get_included_view(INPUT) {
    ...
    let ls_ref = new abap.types.Structure({"name": new abap.types.String({qualifiedName: "NAME"}), "type": new abap.types.ABAPObject({qualifiedName: "CL_ABAP_DATADESCR", RTTIName: "\\CLASS=CL_ABAP_DATADESCR"}), "as_include": new abap.types.Character(1, {"qualifiedName":"ABAP_BOOL","ddicName":"ABAP_BOOL"}), "suffix": new abap.types.String({qualifiedName: "SUFFIX"})}, "abap_componentdescr", undefined, {}, {});
    for await (const unique724 of abap.statements.loop(this.components)) {
      ls_component.set(unique724);
  ...
  async get_ddic_field_list(INPUT) {
    ...
    for await (const unique725 of abap.statements.loop(lt_components)) {
      ls_component.set(unique725);
  ...
  async update_components() {
    ...
    for await (const unique726 of abap.statements.loop(this.mt_refs)) {
      fs_ls_ref_.assign(unique726);
