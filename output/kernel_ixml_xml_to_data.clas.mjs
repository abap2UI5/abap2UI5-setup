...
class kernel_ixml_xml_to_data {
  ...
  static async traverse(INPUT) {
    ...
    if (abap.compare.eq(unique153, abap.Classes['CL_ABAP_TYPEDESCR'].kind_struct)) {
      ...
      let unique154 = 1;
      while (true) {
        abap.builtin.sy.get().index.set(unique154++);
        li_child.set((await li_iterator.get().if_ixml_node_iterator$get_next()));
        if (abap.compare.initial(li_child)) {
          break;
        }
      ...
      let unique155 = 1;
      while (true) {
        abap.builtin.sy.get().index.set(unique155++);
        li_child.set((await li_iterator.get().if_ixml_node_iterator$get_next()));
        if (abap.compare.initial(li_child)) {
          break;
        }
    ...
    } else if (abap.compare.eq(unique153, abap.Classes['CL_ABAP_TYPEDESCR'].kind_ref)) {
      abap.statements.assign({target: fs_any_, source: (iv_ref).dereference()});
