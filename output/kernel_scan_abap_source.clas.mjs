...
class kernel_scan_abap_source {
  ...
  static async call(INPUT) {
    ...
    for await (const unique705 of abap.statements.loop(lt_stokesx)) {
      ls_stokesx.set(unique705);
  ...
  static async pass2(INPUT) {
    ...
    for await (const unique707 of abap.statements.loop(ct_statements)) {
      fs_ls_statement_.assign(unique707);
      ...
      for await (const unique708 of abap.statements.loop(ct_tokens,{from: fs_ls_statement_.get().from,to: fs_ls_statement_.get().to})) {
        ls_token.set(unique708);
      ...
      if (abap.compare.eq(contains_comment, abap.builtin.abap_true) && abap.compare.eq(contains_normal, abap.builtin.abap_true)) {
        ...
        for await (const unique709 of abap.statements.loop(ct_tokens,{from: fs_ls_statement_.get().from,to: fs_ls_statement_.get().to})) {
          ls_token.set(unique709);
          if (abap.compare.eq(ls_token.get().type, kernel_scan_abap_source.gc_token.get().comment)) {
        ...
        for await (const unique710 of abap.statements.loop(lt_delete)) {
          lv_index.set(unique710);
          await abap.statements.deleteInternal(ct_tokens,{index: lv_index});
        }
        for await (const unique711 of abap.statements.loop(lt_insert)) {
          ls_token.set(unique711);
