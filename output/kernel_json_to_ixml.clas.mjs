...
class kernel_json_to_ixml {
  ...
  static async build(INPUT) {
    ...
    while (true) {
      ...
      let unique287 = li_node.get().if_sxml_node$type;
      if (abap.compare.eq(unique287, abap.Classes['IF_SXML_NODE'].if_sxml_node$co_nt_element_open)) {
        ...
        for await (const unique288 of abap.statements.loop(lt_attributes)) {
          li_attribute.set(unique288);
      ...
      } else if (abap.compare.eq(unique287, abap.Classes['IF_SXML_NODE'].if_sxml_node$co_nt_element_close)) {
        await abap.statements.cast(li_close, li_node);
        li_current.set((await li_current.get().if_ixml_node$get_parent()));
      } else if (abap.compare.eq(unique287, abap.Classes['IF_SXML_NODE'].if_sxml_node$co_nt_value)) {
