...
class kernel_create_data_handle {
  ...
  static async call(INPUT) {
    ...
    let unique699 = handle.get().kind;
    if (abap.compare.eq(unique699, abap.Classes['CL_ABAP_TYPEDESCR'].kind_elem)) {
      await this.elem({handle: handle, dref: dref});
    } else if (abap.compare.eq(unique699, abap.Classes['CL_ABAP_TYPEDESCR'].kind_struct)) {
      await this.struct({handle: handle, dref: dref});
    } else if (abap.compare.eq(unique699, abap.Classes['CL_ABAP_TYPEDESCR'].kind_table)) {
      await this.table({handle: handle, dref: dref});
    } else if (abap.compare.eq(unique699, abap.Classes['CL_ABAP_TYPEDESCR'].kind_ref)) {
  ...
  static async struct(INPUT) {
    ...
    let obj = {};
    for await (const unique700 of abap.statements.loop(lt_components)) {
      fs_ls_component_.assign(unique700);
  ...
  static async table(INPUT) {
    ...
    for await (const unique701 of abap.statements.loop(lt_keys,{where: async (I) => {return abap.compare.eq(I.is_primary, abap.builtin.abap_true);},topEquals: {"is_primary": abap.builtin.abap_true}})) {
      fs_ls_key_.assign(unique701);
      ...
      for await (const unique702 of abap.statements.loop(fs_ls_key_.get().components)) {
        lv_component.set(unique702);
  ...
  static async elem(INPUT) {
    ...
    let unique703 = handle.get().type_kind;
    if (abap.compare.eq(unique703, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_float)) {
      abap.statements.createData(dref,{"typeName": "F"});
    } else if (abap.compare.eq(unique703, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_string)) {
      abap.statements.createData(dref,{"typeName": "STRING"});
    } else if (abap.compare.eq(unique703, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_xstring)) {
      abap.statements.createData(dref,{"typeName": "XSTRING"});
    } else if (abap.compare.eq(unique703, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_int)) {
      abap.statements.createData(dref,{"typeName": "I"});
    } else if (abap.compare.eq(unique703, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_utclong)) {
      abap.statements.createData(dref,{"typeName": "UTCLONG"});
    } else if (abap.compare.eq(unique703, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_date)) {
      abap.statements.createData(dref,{"typeName": "D"});
    } else if (abap.compare.eq(unique703, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_hex)) {
      abap.statements.createData(dref,{"typeName": "X","length": handle.get().length});
    } else if (abap.compare.eq(unique703, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_packed)) {
      abap.statements.createData(dref,{"typeName": "P","length": handle.get().length,"decimals": handle.get().decimals});
    } else if (abap.compare.eq(unique703, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_char)) {
    ...
    } else if (abap.compare.eq(unique703, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_num)) {
      lv_half.set(abap.operators.divide(handle.get().length,abap.IntegerFactory.get(2)));
      abap.statements.createData(dref,{"typeName": "N","length": lv_half});
    } else if (abap.compare.eq(unique703, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_time)) {
      abap.statements.createData(dref,{"typeName": "T"});
    } else if (abap.compare.eq(unique703, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_int8)) {
