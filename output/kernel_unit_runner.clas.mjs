...
class kernel_unit_runner {
  ...
  static async get_location(INPUT) {
    ...
    for await (const unique164 of abap.statements.loop(lt_lines)) {
      lv_stack.set(unique164);
  ...
  static async to_json(INPUT) {
    ...
    for await (const unique165 of abap.statements.loop(it_list)) {
      ls_list.set(unique165);
  ...
  static async unique_classes(INPUT) {
    ...
    for await (const unique166 of abap.statements.loop(it_input)) {
      ls_input.set(unique166);
  ...
  static async run(INPUT) {
    ...
    for await (const unique167 of abap.statements.loop(lt_classes)) {
      ls_class.set(unique167);
      ...
      let unique168 = abap.Classes["CLAS-KERNEL_UNIT_RUNNER-"+lv_name.get().trimEnd()];
      if (unique168 === undefined) { unique168 = abap.Classes[lv_name.get().trimEnd()]; }
      if (unique168 === undefined) { throw new abap.Classes['CX_SY_CREATE_OBJECT_ERROR']; }
      lo_obj.set(await (new unique168()).constructor_());
      try {
      ...
      for await (const unique169 of abap.statements.loop(it_input,{where: async (I) => {return abap.compare.eq(I.class_name, ls_class.get().class_name) && abap.compare.eq(I.testclass_name, ls_class.get().testclass_name);},topEquals: {"class_name": ls_class.get().class_name,"testclass_name": ls_class.get().testclass_name}})) {
        ls_input.set(unique169);
