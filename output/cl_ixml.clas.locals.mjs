...
class lcl_named_node_map {
  ...
  async if_ixml_named_node_map$get_named_item_ns(INPUT) {
    ...
    for await (const unique644 of abap.statements.loop(this.mt_list)) {
      li_node.set(unique644);
...
class lcl_node {
  ...
  async if_ixml_element$find_from_name_ns(INPUT) {
    ...
    for await (const unique645 of abap.statements.loop(lt_nodes)) {
      li_top.set(unique645);
      ...
      let unique646 = 1;
      while (true) {
        abap.builtin.sy.get().index.set(unique646++);
  ...
  async if_ixml_element$render(INPUT) {
    ...
    let unique647 = 1;
    while (true) {
      abap.builtin.sy.get().index.set(unique647++);
      li_node.set((await li_iterator.get().if_ixml_node_iterator$get_next()));
      if (abap.compare.initial(li_node)) {
        break;
      }
    ...
    let unique648 = 1;
    while (true) {
      abap.builtin.sy.get().index.set(unique648++);
  ...
  async if_ixml_node$get_depth() {
    ...
    if (abap.compare.eq((await this.mo_children.get().if_ixml_node_list$get_length()), abap.IntegerFactory.get(0))) {
      ...
      let unique649 = 1;
      while (true) {
        abap.builtin.sy.get().index.set(unique649++);
        li_node.set((await li_iterator.get().if_ixml_node_iterator$get_next()));
        if (abap.compare.initial(li_node)) {
          break;
        }
  ...
  async if_ixml_node$get_value() {
    ...
    if (abap.compare.eq((await this.mo_children.get().if_ixml_node_list$get_length()), abap.IntegerFactory.get(0))) {
      ...
      let unique650 = 1;
      while (true) {
        abap.builtin.sy.get().index.set(unique650++);
        li_node.set((await li_iterator.get().if_ixml_node_iterator$get_next()));
        if (abap.compare.initial(li_node)) {
          break;
        }
...
class lcl_parser {
  ...
  async if_ixml_parser$parse() {
    ...
    while (abap.compare.initial(lv_xml) === false) {
      abap.builtin.sy.get().index.set(unique651++);
  ...
  async parse_attributes(INPUT) {
    ...
    let unique652 = 1;
    while (true) {
      abap.builtin.sy.get().index.set(unique652++);
