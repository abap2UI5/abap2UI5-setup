  ...
  static async call(INPUT) {
    ...
    if (abap.compare.initial(lv_source) === false) {
      if (abap.compare.eq(lv_source.getOffset({length: 1}), new abap.types.Character(1).set('<'))) {
        ...
        const unique683 = await (new abap.Classes['CX_XSLT_FORMAT_ERROR']()).constructor_();
        unique683.EXTRA_CX = {"INTERNAL_FILENAME": "kernel_call_transformation.clas.abap","INTERNAL_LINE": 68};
        throw unique683;
      }
    }
    ...
    if (INPUT.resultXML && INPUT.resultXML.constructor.name === "String") {
      ...
      if (abap.compare.initial(lv_source) && abap.compare.initial(kernel_call_transformation.mi_doc)) {
        const unique684 = await (new abap.Classes['CX_XSLT_RUNTIME_ERROR']()).constructor_();
        unique684.EXTRA_CX = {"INTERNAL_FILENAME": "kernel_call_transformation.clas.abap","INTERNAL_LINE": 128};
        throw unique684;
      }
      if (INPUT.result.constructor.name === "Table") {
        lt_rtab = INPUT.result;
        for await (const unique685 of abap.statements.loop(lt_rtab)) {
          ls_rtab.set(unique685);
