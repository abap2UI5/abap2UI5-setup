...
class cl_abap_conv_in_ce {
  ...
  static async create(INPUT) {
    ...
    let unique620 = encoding;
    if (abap.compare.eq(unique620, new abap.types.Character(6).set('UTF-16'))) {
      ret.get().mv_js_encoding.set(new abap.types.Character(8).set('utf-16le'));
    } else if (abap.compare.eq(unique620, new abap.types.Character(5).set('UTF-8'))) {
      ret.get().mv_js_encoding.set(new abap.types.Character(4).set('utf8'));
    } else if (abap.compare.eq(unique620, new abap.types.Character(4).set('4103'))) {
      ret.get().mv_js_encoding.set(new abap.types.Character(8).set('utf-16le'));
    } else {
  ...
  async convert(INPUT) {
    ...
    if (abap.compare.eq(lv_error, abap.builtin.abap_true)) {
      const unique621 = await (new abap.Classes['CX_SY_CONVERSION_CODEPAGE']()).constructor_();
      unique621.EXTRA_CX = {"INTERNAL_FILENAME": "cl_abap_conv_in_ce.clas.abap","INTERNAL_LINE": 118};
      throw unique621;
    }
  }
  async read(INPUT) {
