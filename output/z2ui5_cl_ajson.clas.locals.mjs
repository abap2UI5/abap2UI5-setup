  ...
  static async string_to_xstring_utf8(INPUT) {
    ...
    try {
      ...
      if ((abap.Classes['CX_SY_DYN_CALL_ILLEGAL_CLASS'] && e instanceof abap.Classes['CX_SY_DYN_CALL_ILLEGAL_CLASS'])) {
        if (abap.Classes[lv_out_ce.get().trimEnd()] === undefined && abap.Classes['CX_SY_DYN_CALL_ILLEGAL_CLASS'.trimEnd()] === undefined) { throw "CX_SY_DYN_CALL_ILLEGAL_CLASS not found"; }
        if (abap.Classes[lv_out_ce.get().trimEnd()] === undefined) { throw new abap.Classes['CX_SY_DYN_CALL_ILLEGAL_CLASS'.trimEnd()](); }
  ...
  async get_node_type(INPUT) {
    ...
    async value_to_abap(INPUT) {
      ...
      if (abap.compare.eq(unique12, abap.Classes['Z2UI5_IF_AJSON_TYPES'].z2ui5_if_ajson_types$node_type.get().null)) {
        ...
        if (abap.compare.eq(is_node_type.get().type_kind, abap.Classes['CLAS-Z2UI5_CL_AJSON-LIF_KIND'].lif_kind$date) && abap.compare.initial(is_node.get().value) === false) {
          fs_container_.set((await this.to_date({iv_value: is_node.get().value})));
        } else if (abap.compare.eq(is_node_type.get().type_kind, abap.Classes['CLAS-Z2UI5_CL_AJSON-LIF_KIND'].lif_kind$time) && abap.compare.initial(is_node.get().value) === false) {
          fs_container_.set((await this.to_time({iv_value: is_node.get().value})));
        } else if (abap.compare.eq(is_node_type.get().type_kind, abap.Classes['CLAS-Z2UI5_CL_AJSON-LIF_KIND'].lif_kind$packed) && abap.compare.initial(is_node.get().value) === false) {
