...
class kernel_create_data_handle {
  ...
  static async call(INPUT) {
    ...
    let unique142 = handle.get().kind;
    if (abap.compare.eq(unique142, abap.Classes['CL_ABAP_TYPEDESCR'].kind_elem)) {
      await this.elem({handle: handle, dref: dref});
    } else if (abap.compare.eq(unique142, abap.Classes['CL_ABAP_TYPEDESCR'].kind_struct)) {
  ...
  static async struct(INPUT) {
    ...
    let obj = {};
    for await (const unique143 of abap.statements.loop(lt_components)) {
      fs_ls_component_.assign(unique143);
  ...
  static async table(INPUT) {
    ...
    for await (const unique144 of abap.statements.loop(lt_keys,{where: async (I) => {return abap.compare.eq(I.is_primary, abap.builtin.abap_true);},topEquals: {"is_primary": abap.builtin.abap_true}})) {
      fs_ls_key_.assign(unique144);
      ...
      for await (const unique145 of abap.statements.loop(fs_ls_key_.get().components)) {
        lv_component.set(unique145);
  ...
  static async elem(INPUT) {
    ...
    let unique146 = handle.get().type_kind;
    if (abap.compare.eq(unique146, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_float)) {
      abap.statements.createData(dref,{"typeName": "F"});
    } else if (abap.compare.eq(unique146, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_string)) {
      abap.statements.createData(dref,{"typeName": "STRING"});
    } else if (abap.compare.eq(unique146, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_xstring)) {
      abap.statements.createData(dref,{"typeName": "XSTRING"});
    } else if (abap.compare.eq(unique146, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_int)) {
      abap.statements.createData(dref,{"typeName": "I"});
    } else if (abap.compare.eq(unique146, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_date)) {
      abap.statements.createData(dref,{"typeName": "D"});
    } else if (abap.compare.eq(unique146, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_hex)) {
      abap.statements.createData(dref,{"typeName": "X","length": handle.get().length});
    } else if (abap.compare.eq(unique146, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_packed)) {
      abap.statements.createData(dref,{"typeName": "P","length": handle.get().length,"decimals": handle.get().decimals});
    } else if (abap.compare.eq(unique146, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_char)) {
    ...
    } else if (abap.compare.eq(unique146, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_num)) {
      lv_half.set(abap.operators.divide(handle.get().length,abap.IntegerFactory.get(2)));
      abap.statements.createData(dref,{"typeName": "N","length": lv_half});
    } else if (abap.compare.eq(unique146, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_time)) {
      abap.statements.createData(dref,{"typeName": "T"});
    } else if (abap.compare.eq(unique146, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_int8)) {
