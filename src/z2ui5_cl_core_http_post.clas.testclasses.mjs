...
class ltcl_test_handler_post {
  ...
  async load_startup_app() {
    ...
    let temp2 = new abap.types.ABAPObject({qualifiedName: "Z2UI5_CL_CORE_APP_STARTUP", RTTIName: "\\CLASS=Z2UI5_CL_CORE_APP_STARTUP"});
    ...
    await abap.statements.cast(temp2, lo_post.get().mo_action.get().mo_app.get().mo_app);
    lo_startup.set(temp2);
  }
}
