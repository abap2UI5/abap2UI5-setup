...
class lcl_heap {
  ...
  async add(INPUT) {
    ...
    for await (const unique257 of abap.statements.loop(lo_descr.get().interfaces)) {
      ls_interface.set(unique257);
    ...
    if (abap.compare.eq(is_serializable, abap.builtin.abap_true)) {
      ...
      for await (const unique258 of abap.statements.loop(lo_descr.get().attributes,{where: async (I) => {return abap.compare.eq(I.is_class, abap.builtin.abap_false);},topEquals: {"is_class": abap.builtin.abap_false}})) {
        ls_attribute.set(unique258);
  ...
  class lcl_data_to_xml {
    ...
    async run(INPUT) {
      ...
      if (abap.compare.eq(unique259, abap.Classes['CL_ABAP_TYPEDESCR'].kind_struct)) {
        ...
        rv_xml.set(abap.operators.concat(rv_xml,new abap.types.String().set(`</${abap.templateFormatting(iv_name)}>`)));
      } else if (abap.compare.eq(unique259, abap.Classes['CL_ABAP_TYPEDESCR'].kind_ref)) {
        let unique262 = lo_type.get().type_kind;
        if (abap.compare.eq(unique262, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_oref)) {
