...
class kernel_ixml_json_to_data {
  ...
  static async traverse(INPUT) {
    ...
    let unique145 = lo_type.get().kind;
    if (abap.compare.eq(unique145, abap.Classes['CL_ABAP_TYPEDESCR'].kind_struct)) {
      ...
      let unique146 = 1;
      while (true) {
        abap.builtin.sy.get().index.set(unique146++);
      ...
      abap.builtin.sy.get().index.set(indexBackup1);
    } else if (abap.compare.eq(unique145, abap.Classes['CL_ABAP_TYPEDESCR'].kind_elem)) {
    ...
    } else if (abap.compare.eq(unique145, abap.Classes['CL_ABAP_TYPEDESCR'].kind_table)) {
