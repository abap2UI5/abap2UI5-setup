...
class kernel_ixml_xml_to_data {
  ...
  static async find_href_in_heap(INPUT) {
    ...
    let unique691 = 1;
    while (true) {
      abap.builtin.sy.get().index.set(unique691++);
  ...
  static async traverse(INPUT) {
    ...
    if (abap.compare.eq(unique692, abap.Classes['CL_ABAP_TYPEDESCR'].kind_struct)) {
      ...
      let unique693 = 1;
      while (true) {
        abap.builtin.sy.get().index.set(unique693++);
        li_child.set((await li_iterator.get().if_ixml_node_iterator$get_next()));
        if (abap.compare.initial(li_child)) {
          break;
        }
      ...
      let unique694 = 1;
      while (true) {
        abap.builtin.sy.get().index.set(unique694++);
        li_child.set((await li_iterator.get().if_ixml_node_iterator$get_next()));
        if (abap.compare.initial(li_child)) {
          break;
        }
    ...
    } else if (abap.compare.eq(unique692, abap.Classes['CL_ABAP_TYPEDESCR'].kind_ref)) {
      abap.statements.assign({target: fs_any_, source: iv_ref.dereference()});
      if (abap.compare.initial(fs_any_)) {
        ...
        if (abap.compare.eq(lv_value.getOffset({length: 2}), new abap.types.Character(2).set('#o'))) {
          ...
          let unique695 = 1;
          while (true) {
            abap.builtin.sy.get().index.set(unique695++);
            li_child.set((await li_iterator.get().if_ixml_node_iterator$get_next()));
            if (abap.compare.initial(li_child)) {
              break;
            }
