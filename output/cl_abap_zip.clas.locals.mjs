...
class lcl_stream {
  ...
  async append_crc(INPUT) {
    ...
    if (abap.compare.eq(abap.builtin.xstrlen({val: lcl_stream.crc32_map}), abap.IntegerFactory.get(0))) {
      ...
      const unique605 = new abap.types.Integer().set(256).get();
      for (let unique606 = 0; unique606 < unique605; unique606++) {
        abap.builtin.sy.get().index.set(unique606 + 1);
        ...
        const unique607 = abap.IntegerFactory.get(8).get();
        for (let unique608 = 0; unique608 < unique607; unique608++) {
          abap.builtin.sy.get().index.set(unique608 + 1);
    ...
    const unique609 = len.get();
    for (let unique610 = 0; unique610 < unique609; unique610++) {
      abap.builtin.sy.get().index.set(unique610 + 1);
