...
class kernel_scan_abap_source {
  ...
  static async call(INPUT) {
    ...
    for await (const unique289 of abap.statements.loop(lt_stokesx)) {
      ls_stokesx.set(unique289);
  ...
  static async pass2(INPUT) {
    ...
    for await (const unique291 of abap.statements.loop(ct_statements)) {
      fs_ls_statement_.assign(unique291);
      ...
      for await (const unique292 of abap.statements.loop(ct_tokens,{from: fs_ls_statement_.get().from,to: fs_ls_statement_.get().to})) {
        ls_token.set(unique292);
      ...
      if (abap.compare.eq(contains_comment, abap.builtin.abap_true) && abap.compare.eq(contains_normal, abap.builtin.abap_true)) {
        ...
        for await (const unique293 of abap.statements.loop(ct_tokens,{from: fs_ls_statement_.get().from,to: fs_ls_statement_.get().to})) {
          ls_token.set(unique293);
          if (abap.compare.eq(ls_token.get().type, kernel_scan_abap_source.gc_token.get().comment)) {
        ...
        for await (const unique294 of abap.statements.loop(lt_delete)) {
          lv_index.set(unique294);
          await abap.statements.deleteInternal(ct_tokens,{index: lv_index});
        }
        for await (const unique295 of abap.statements.loop(lt_insert)) {
          ls_token.set(unique295);
