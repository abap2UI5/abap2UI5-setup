...
class cl_osql_test_environment {
  ...
  async validate() {
    ...
    for await (const unique784 of abap.statements.loop(this.mt_tables)) {
      lv_table.set(unique784);
      try {
  ...
  async initialize() {
    ...
    for await (const unique785 of abap.statements.loop(this.mt_tables)) {
      lv_table.set(unique785);
  ...
  async if_osql_test_environment$clear_doubles() {
    let lv_table = new abap.types.Character(30, {"qualifiedName":"abap_compname"});
    for await (const unique786 of abap.statements.loop(this.mt_tables)) {
      lv_table.set(unique786);
      lv_table.set(abap.builtin.to_lower({val: lv_table}));
