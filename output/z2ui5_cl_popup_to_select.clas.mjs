...
class z2ui5_cl_popup_to_select {
  ...
  async on_event() {
    ...
    if (abap.compare.eq(unique187, new abap.types.Character(7).set('CONFIRM'))) {
    ...
    } else if (abap.compare.eq(unique187, new abap.types.Character(6).set('SEARCH'))) {
  ...
  async set_output_table() {
    ...
    for await (const unique188 of abap.statements.loop(fs_tab_)) {
      fs_row_.assign(unique188);
    ...
    async on_event_confirm() {
      ...
      for await (const unique189 of abap.statements.loop(fs_tab_)) {
        fs_row_selected_.assign(unique189);
        ...
        async on_event_search() {
          ...
          lt_comp.set((await abap.Classes['Z2UI5_CL_UTIL'].rtti_get_t_attri_by_struc({val: fs_tab_out_})));
          for await (const unique190 of abap.statements.loop(fs_tab_out_)) {
            fs_row2_.assign(unique190);
