...
class kernel_unit_runner {
  ...
  static async get_location(INPUT) {
    ...
    for await (const unique355 of abap.statements.loop(lt_lines)) {
      lv_stack.set(unique355);
  ...
  static async to_json(INPUT) {
    ...
    for await (const unique356 of abap.statements.loop(it_list)) {
      ls_list.set(unique356);
  ...
  static async unique_classes(INPUT) {
    ...
    for await (const unique357 of abap.statements.loop(it_input)) {
      ls_input.set(unique357);
  ...
  static async run(INPUT) {
    ...
    for await (const unique358 of abap.statements.loop(lt_classes)) {
      ls_class.set(unique358);
      ...
      let unique359 = abap.Classes["CLAS-KERNEL_UNIT_RUNNER-"+lv_name.get().trimEnd()];
      if (unique359 === undefined) { unique359 = abap.Classes[lv_name.get().trimEnd()]; }
      if (unique359 === undefined) { throw new abap.Classes['CX_SY_CREATE_OBJECT_ERROR']; }
      lo_obj.set(await (new unique359()).constructor_());
      try {
      ...
      for await (const unique360 of abap.statements.loop(it_input,{where: async (I) => {return abap.compare.eq(I.class_name, ls_class.get().class_name) && abap.compare.eq(I.testclass_name, ls_class.get().testclass_name);},topEquals: {"class_name": ls_class.get().class_name,"testclass_name": ls_class.get().testclass_name}})) {
        ls_input.set(unique360);
