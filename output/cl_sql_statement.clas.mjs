...
class cl_sql_statement {
  ...
  async execute_update(INPUT) {
    ...
    if (abap.compare.initial(lv_sql_message) === false) {
      const unique612 = await (new abap.Classes['CX_SQL_EXCEPTION']()).constructor_();
      unique612.EXTRA_CX = {"INTERNAL_FILENAME": "cl_sql_statement.clas.abap","INTERNAL_LINE": 48};
      throw unique612;
    }
    try {
    ...
    if (abap.compare.initial(lv_sql_message) === false) {
      const unique613 = await (new abap.Classes['CX_SQL_EXCEPTION']()).constructor_();
      unique613.EXTRA_CX = {"INTERNAL_FILENAME": "cl_sql_statement.clas.abap","INTERNAL_LINE": 57};
      throw unique613;
    }
  }
  async execute_query(INPUT) {
    ...
    if (abap.compare.initial(lv_sql_message) === false) {
      const unique614 = await (new abap.Classes['CX_SQL_EXCEPTION']()).constructor_();
      unique614.EXTRA_CX = {"INTERNAL_FILENAME": "cl_sql_statement.clas.abap","INTERNAL_LINE": 71};
      throw unique614;
    }
    ...
    if (abap.compare.initial(lv_sql_message) === false) {
      const unique615 = await (new abap.Classes['CX_SQL_EXCEPTION']()).constructor_();
      unique615.EXTRA_CX = {"INTERNAL_FILENAME": "cl_sql_statement.clas.abap","INTERNAL_LINE": 84};
      throw unique615;
    }
    return result_set;
  }
