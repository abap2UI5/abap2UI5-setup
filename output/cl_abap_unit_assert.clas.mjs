...
class cl_abap_unit_assert {
  ...
  static async compare_tables(INPUT) {
    ...
    if (abap.compare.ne(abap.builtin.lines({val: act}), abap.builtin.lines({val: exp}))) {
      const unique749 = await (new abap.Classes['KERNEL_CX_ASSERT']()).constructor_({msg: new abap.types.String().set(`Expected table to contain ${abap.templateFormatting(abap.builtin.lines({val: exp}))} rows, got ${abap.templateFormatting(abap.builtin.lines({val: act}))}`)});
      unique749.EXTRA_CX = {"INTERNAL_FILENAME": "cl_abap_unit_assert.clas.abap","INTERNAL_LINE": 162};
      throw unique749;
    }
    ...
    if (abap.compare.eq(type1.get().table_kind, abap.Classes['CL_ABAP_TABLEDESCR'].tablekind_hashed) || abap.compare.eq(type2.get().table_kind, abap.Classes['CL_ABAP_TABLEDESCR'].tablekind_hashed)) {
      for await (const unique750 of abap.statements.loop(fs_tab1_)) {
        fs_row1_.assign(unique750);
        lv_match.set(abap.builtin.abap_false);
        for await (const unique751 of abap.statements.loop(fs_tab2_)) {
          fs_row2_.assign(unique751);
          try {
        ...
        if (abap.compare.eq(lv_match, abap.builtin.abap_false)) {
          const unique752 = await (new abap.Classes['KERNEL_CX_ASSERT']()).constructor_({msg: new abap.types.String().set(`Hashed table contents differs`)});
          unique752.EXTRA_CX = {"INTERNAL_FILENAME": "cl_abap_unit_assert.clas.abap","INTERNAL_LINE": 188};
          throw unique752;
        }
      }
    } else {
      const indexBackup1 = abap.builtin.sy.get().index.get();
      const unique753 = abap.builtin.lines({val: act}).get();
  ...
  static async assert_text_matches(INPUT) {
    ...
    if (abap.compare.eq(lv_match, abap.builtin.abap_false)) {
      const unique755 = await (new abap.Classes['KERNEL_CX_ASSERT']()).constructor_({expected: pattern, actual: text, msg: msg});
      unique755.EXTRA_CX = {"INTERNAL_FILENAME": "cl_abap_unit_assert.clas.abap","INTERNAL_LINE": 214};
      throw unique755;
    }
  }
  async abort(INPUT) {
  ...
  static async assert_bound(INPUT) {
    ...
    if (abap.compare.initial(act)) {
      const unique756 = await (new abap.Classes['KERNEL_CX_ASSERT']()).constructor_({msg: new abap.types.String().set(`Expected value to be bound`)});
      unique756.EXTRA_CX = {"INTERNAL_FILENAME": "cl_abap_unit_assert.clas.abap","INTERNAL_LINE": 228};
      throw unique756;
    }
  }
  ...
  static async assert_not_bound(INPUT) {
    ...
    if (abap.compare.initial(act) === false) {
      const unique757 = await (new abap.Classes['KERNEL_CX_ASSERT']()).constructor_({msg: new abap.types.String().set(`Expected value to not be bound`)});
      unique757.EXTRA_CX = {"INTERNAL_FILENAME": "cl_abap_unit_assert.clas.abap","INTERNAL_LINE": 236};
      throw unique757;
    }
  }
  ...
  static async assert_char_cp(INPUT) {
    ...
    if (abap.compare.np(act, exp)) {
      const unique758 = await (new abap.Classes['KERNEL_CX_ASSERT']()).constructor_({expected: exp, actual: act, msg: msg});
      unique758.EXTRA_CX = {"INTERNAL_FILENAME": "cl_abap_unit_assert.clas.abap","INTERNAL_LINE": 244};
      throw unique758;
    }
  }
  ...
  static async assert_char_np(INPUT) {
    ...
    if (abap.compare.cp(act, exp)) {
      const unique759 = await (new abap.Classes['KERNEL_CX_ASSERT']()).constructor_({msg: new abap.types.String().set(`Actual: ${abap.templateFormatting(act)}`)});
      unique759.EXTRA_CX = {"INTERNAL_FILENAME": "cl_abap_unit_assert.clas.abap","INTERNAL_LINE": 254};
      throw unique759;
    }
  }
  async fail(INPUT) {
  ...
  static async fail(INPUT) {
    ...
    if (INPUT && INPUT.level) {level.set(INPUT.level);}
    let detail = INPUT?.detail || new abap.types.Character();
    const unique760 = await (new abap.Classes['KERNEL_CX_ASSERT']()).constructor_({msg: msg});
    unique760.EXTRA_CX = {"INTERNAL_FILENAME": "cl_abap_unit_assert.clas.abap","INTERNAL_LINE": 261};
    throw unique760;
  }
  async skip(INPUT) {
  ...
  static async assert_differs(INPUT) {
    ...
    try {
      ...
      const unique761 = await (new abap.Classes['KERNEL_CX_ASSERT']()).constructor_({msg: new abap.types.String().set(`Expected different values`), actual: act, expected: exp});
      unique761.EXTRA_CX = {"INTERNAL_FILENAME": "cl_abap_unit_assert.clas.abap","INTERNAL_LINE": 275};
      throw unique761;
    } catch (e) {
      if ((abap.Classes['KERNEL_CX_ASSERT'] && e instanceof abap.Classes['KERNEL_CX_ASSERT'])) {
        return;
  ...
  static async assert_true(INPUT) {
    ...
    if (abap.compare.ne(act, abap.builtin.abap_true)) {
      const unique762 = await (new abap.Classes['KERNEL_CX_ASSERT']()).constructor_({msg: new abap.types.String().set(`Expected abap_true`)});
      unique762.EXTRA_CX = {"INTERNAL_FILENAME": "cl_abap_unit_assert.clas.abap","INTERNAL_LINE": 287};
      throw unique762;
    }
  }
  ...
  static async assert_false(INPUT) {
    ...
    if (abap.compare.ne(act, abap.builtin.abap_false)) {
      const unique763 = await (new abap.Classes['KERNEL_CX_ASSERT']()).constructor_({msg: new abap.types.String().set(`Expected abap_false`)});
      unique763.EXTRA_CX = {"INTERNAL_FILENAME": "cl_abap_unit_assert.clas.abap","INTERNAL_LINE": 295};
      throw unique763;
    }
  }
  ...
  static async assert_equals(INPUT) {
    ...
    if (abap.compare.ca(type1, new abap.types.Character(11).set('CgyIFPDTXN8'))) {
      if (abap.compare.initial(type2) === false) {
        if (abap.compare.na(type2, new abap.types.Character(11).set('CgyIFPDTXN8'))) {
          const unique764 = await (new abap.Classes['KERNEL_CX_ASSERT']()).constructor_({msg: new abap.types.String().set(`Unexpected types`)});
          unique764.EXTRA_CX = {"INTERNAL_FILENAME": "cl_abap_unit_assert.clas.abap","INTERNAL_LINE": 316};
          throw unique764;
        }
      }
    } else if (abap.compare.initial(type1) === false && abap.compare.initial(type2) === false) {
      if (abap.compare.ne(type1, type2)) {
        const unique765 = await (new abap.Classes['KERNEL_CX_ASSERT']()).constructor_({msg: new abap.types.String().set(`Unexpected types`)});
        unique765.EXTRA_CX = {"INTERNAL_FILENAME": "cl_abap_unit_assert.clas.abap","INTERNAL_LINE": 324};
        throw unique765;
      }
    }
    if (abap.compare.eq(type1, new abap.types.Character(1).set('h'))) {
      ...
      if (abap.compare.ge(diff, tol)) {
        const unique766 = await (new abap.Classes['KERNEL_CX_ASSERT']()).constructor_();
        unique766.EXTRA_CX = {"INTERNAL_FILENAME": "cl_abap_unit_assert.clas.abap","INTERNAL_LINE": 339};
        throw unique766;
      }
      ...
      const unique767 = await (new abap.Classes['KERNEL_CX_ASSERT']()).constructor_({msg: lv_msg, actual: lv_act, expected: lv_exp});
      unique767.EXTRA_CX = {"INTERNAL_FILENAME": "cl_abap_unit_assert.clas.abap","INTERNAL_LINE": 353};
      throw unique767;
    }
  }
  ...
  static async assert_not_initial(INPUT) {
    ...
    if (abap.compare.initial(act)) {
      ...
      const unique768 = await (new abap.Classes['KERNEL_CX_ASSERT']()).constructor_({msg: lv_msg});
      unique768.EXTRA_CX = {"INTERNAL_FILENAME": "cl_abap_unit_assert.clas.abap","INTERNAL_LINE": 368};
      throw unique768;
    }
  }
  ...
  static async assert_initial(INPUT) {
    ...
    if (abap.compare.initial(act) === false) {
      ...
      const unique769 = await (new abap.Classes['KERNEL_CX_ASSERT']()).constructor_({msg: lv_msg});
      unique769.EXTRA_CX = {"INTERNAL_FILENAME": "cl_abap_unit_assert.clas.abap","INTERNAL_LINE": 381};
      throw unique769;
    }
  }
  ...
  static async assert_subrc(INPUT) {
    ...
    if (abap.compare.ne(act, exp)) {
      ...
      const unique770 = await (new abap.Classes['KERNEL_CX_ASSERT']()).constructor_({msg: lv_msg});
      unique770.EXTRA_CX = {"INTERNAL_FILENAME": "cl_abap_unit_assert.clas.abap","INTERNAL_LINE": 394};
      throw unique770;
    }
  }
  ...
  static async assert_number_between(INPUT) {
    ...
    if (abap.compare.lt(number, lower) || abap.compare.gt(number, upper)) {
      const unique771 = await (new abap.Classes['KERNEL_CX_ASSERT']()).constructor_();
      unique771.EXTRA_CX = {"INTERNAL_FILENAME": "cl_abap_unit_assert.clas.abap","INTERNAL_LINE": 402};
      throw unique771;
    }
  }
}
