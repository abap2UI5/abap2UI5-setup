...
class $ui2$cl_json {
  ...
  async serialize_int(INPUT) {
    ...
    let unique252 = lo_type.get().kind;
    if (abap.compare.eq(unique252, abap.Classes['CL_ABAP_TYPEDESCR'].kind_elem)) {
      let unique253 = lo_type.get().type_kind;
      if (abap.compare.eq(unique253, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_char)) {
      ...
      } else if (abap.compare.eq(unique253, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_string)) {
      ...
      } else if (abap.compare.eq(unique253, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_int)) {
        r_json.set(new abap.types.String().set(`${abap.templateFormatting(data)}`));
      } else if (abap.compare.eq(unique253, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_num)) {
      ...
      } else if (abap.compare.eq(unique253, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_packed)) {
      ...
      } else if (abap.compare.eq(unique253, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_date)) {
        r_json.set(new abap.types.String().set(`"${abap.templateFormatting(data,{"date":"iso"})}"`));
      } else if (abap.compare.eq(unique253, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_time)) {
        r_json.set(new abap.types.String().set(`"${abap.templateFormatting(data,{"time":"iso"})}"`));
      } else {
        r_json.set(data);
      }
    } else if (abap.compare.eq(unique252, abap.Classes['CL_ABAP_TYPEDESCR'].kind_table)) {
      ...
      for await (const unique254 of abap.statements.loop(fs_tab_)) {
        fs_any_.assign(unique254);
      ...
      r_json.set(abap.operators.concat(r_json,new abap.types.Character(1).set(']')));
    } else if (abap.compare.eq(unique252, abap.Classes['CL_ABAP_TYPEDESCR'].kind_struct)) {
      await abap.statements.cast(lo_struct, lo_type);
      lt_components.set((await lo_struct.get().get_components()));
      r_json.set(new abap.types.Character(1).set('{'));
      for await (const unique255 of abap.statements.loop(lt_components)) {
        fs_ls_component_.assign(unique255);
      ...
      r_json.set(abap.operators.concat(r_json,new abap.types.Character(1).set('}')));
    } else if (abap.compare.eq(unique252, abap.Classes['CL_ABAP_TYPEDESCR'].kind_ref)) {
      if (abap.compare.initial(data)) {
  ...
  static async _deserialize(INPUT) {
    ...
    let unique256 = io_type.get().kind;
    if (abap.compare.eq(unique256, abap.Classes['CL_ABAP_TYPEDESCR'].kind_elem)) {
    ...
    } else if (abap.compare.eq(unique256, abap.Classes['CL_ABAP_TYPEDESCR'].kind_table)) {
      ...
      for await (const unique257 of abap.statements.loop(lt_members)) {
        lv_member.set(unique257);
    ...
    } else if (abap.compare.eq(unique256, abap.Classes['CL_ABAP_TYPEDESCR'].kind_struct)) {
      await abap.statements.cast(lo_struct, io_type);
      lt_components.set((await lo_struct.get().get_components()));
      for await (const unique258 of abap.statements.loop(lt_components)) {
        fs_ls_component_.assign(unique258);
        abap.statements.assign({component: fs_ls_component_.get().name, target: fs_any_, source: data});
        abap.statements.assert(abap.compare.eq(abap.builtin.sy.get().subrc, abap.IntegerFactory.get(0)));
        let unique259 = pretty_name;
        if (abap.compare.eq(unique259, $ui2$cl_json.pretty_mode.get().camel_case)) {
    ...
    } else if (abap.compare.eq(unique256, abap.Classes['CL_ABAP_TYPEDESCR'].kind_ref)) {
      ...
      if (abap.compare.initial(data)) {
        ...
        if (abap.compare.gt(abap.builtin.lines({val: lt_members}), abap.IntegerFactory.get(0)) && abap.compare.eq(lv_type, new abap.types.Character(6).set('object'))) {
          ...
          for await (const unique260 of abap.statements.loop(lt_members)) {
            lv_member.set(unique260);
          ...
          let unique261 = lv_type;
          if (abap.compare.eq(unique261, new abap.types.Character(3).set('num'))) {
            lv_value.set((await $ui2$cl_json.mo_parsed.get().value_string({iv_path: prefix})));
          ...
          } else if (abap.compare.eq(unique261, new abap.types.Character(4).set('bool'))) {
            if (abap.Classes['KERNEL_CREATE_DATA_HANDLE'] === undefined) throw new Error("CreateData, kernel class missing");
          ...
          } else if (abap.compare.eq(unique261, new abap.types.Character(3).set('str'))) {
            if (abap.Classes['KERNEL_CREATE_DATA_HANDLE'] === undefined) throw new Error("CreateData, kernel class missing");
