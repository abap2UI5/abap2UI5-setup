...
class kernel_unit_runner {
  ...
  static async get_location(INPUT) {
    ...
    for await (const unique360 of abap.statements.loop(lt_lines)) {
      lv_stack.set(unique360);
  ...
  static async to_json(INPUT) {
    ...
    for await (const unique361 of abap.statements.loop(it_list)) {
      ls_list.set(unique361);
  ...
  static async unique_classes(INPUT) {
    ...
    for await (const unique362 of abap.statements.loop(it_input)) {
      ls_input.set(unique362);
  ...
  static async run(INPUT) {
    ...
    for await (const unique363 of abap.statements.loop(lt_classes)) {
      ls_class.set(unique363);
      ...
      let unique364 = abap.Classes["CLAS-KERNEL_UNIT_RUNNER-"+lv_name.get().trimEnd()];
      if (unique364 === undefined) { unique364 = abap.Classes[lv_name.get().trimEnd()]; }
      if (unique364 === undefined) { throw new abap.Classes['CX_SY_CREATE_OBJECT_ERROR']; }
      lo_obj.set(await (new unique364()).constructor_());
      try {
      ...
      for await (const unique365 of abap.statements.loop(it_input,{where: async (I) => {return abap.compare.eq(I.class_name, ls_class.get().class_name) && abap.compare.eq(I.testclass_name, ls_class.get().testclass_name);},topEquals: {"class_name": ls_class.get().class_name,"testclass_name": ls_class.get().testclass_name}})) {
        ls_input.set(unique365);
