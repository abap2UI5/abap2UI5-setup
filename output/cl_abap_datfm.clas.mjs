...
class cl_abap_datfm {
  ...
  static async conv_date_ext_to_int(INPUT) {
    ...
    if (abap.compare.ne(im_datfmdes, cl_abap_datfm.ddmmyyyy_dot_seperated) && abap.compare.ne(im_datfmdes, cl_abap_datfm.yyyymmdd_dot_seperated)) {
      const unique217 = await (new abap.Classes['CX_ABAP_DATFM']()).constructor_();
      unique217.EXTRA_CX = {"INTERNAL_FILENAME": "cl_abap_datfm.clas.abap","INTERNAL_LINE": 50};
      throw unique217;
    }
    ...
    const unique218 = await (new abap.Classes['CX_ABAP_DATFM']()).constructor_();
    unique218.EXTRA_CX = {"INTERNAL_FILENAME": "cl_abap_datfm.clas.abap","INTERNAL_LINE": 74};
    throw unique218;
  }
