...
class kernel_unit_runner {
  ...
  static async run(INPUT) {
    ...
    for await (const unique171 of abap.statements.loop(lt_classes)) {
      ...
      let unique172 = abap.Classes["CLAS-KERNEL_UNIT_RUNNER-"+lv_name.get().trimEnd()];
      ...
      lo_obj.set(await (new unique172()).constructor_());
      try {
      ...
      for await (const unique173 of abap.statements.loop(it_input,{where: async (I) => {return abap.compare.eq(I.class_name, ls_class.get().class_name) && abap.compare.eq(I.testclass_name, ls_class.get().testclass_name);},topEquals: {"class_name": ls_class.get().class_name,"testclass_name": ls_class.get().testclass_name}})) {
