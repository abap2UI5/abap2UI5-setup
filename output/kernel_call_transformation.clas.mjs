  ...
  static async call(INPUT) {
    ...
    if (abap.compare.initial(lv_source) === false) {
      if (abap.compare.eq(lv_source.getOffset({length: 1}), new abap.types.Character(1).set('<'))) {
        ...
        const unique139 = await (new abap.Classes['CX_XSLT_FORMAT_ERROR']()).constructor_();
        unique139.EXTRA_CX = {"INTERNAL_FILENAME": "kernel_call_transformation.clas.abap","INTERNAL_LINE": 67};
        throw unique139;
      }
    }
    ...
    if (abap.compare.initial(lv_source) && abap.compare.initial(kernel_call_transformation.mi_doc)) {
      const unique140 = await (new abap.Classes['CX_XSLT_RUNTIME_ERROR']()).constructor_();
      unique140.EXTRA_CX = {"INTERNAL_FILENAME": "kernel_call_transformation.clas.abap","INTERNAL_LINE": 112};
      throw unique140;
    }
    if (INPUT.result.constructor.name === "Table") {
      lt_rtab = INPUT.result;
      for await (const unique141 of abap.statements.loop(lt_rtab)) {
        ls_rtab.set(unique141);
