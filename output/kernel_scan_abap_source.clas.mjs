...
class kernel_scan_abap_source {
  ...
  static async call(INPUT) {
    ...
    for await (const unique295 of abap.statements.loop(lt_stokesx)) {
      ls_stokesx.set(unique295);
  ...
  static async pass2(INPUT) {
    ...
    for await (const unique297 of abap.statements.loop(ct_statements)) {
      fs_ls_statement_.assign(unique297);
      ...
      if (abap.compare.eq(contains_comment, abap.builtin.abap_true) && abap.compare.eq(contains_normal, abap.builtin.abap_true)) {
        ...
        for await (const unique300 of abap.statements.loop(lt_delete)) {
          lv_index.set(unique300);
          await abap.statements.deleteInternal(ct_tokens,{index: lv_index});
        }
        for await (const unique301 of abap.statements.loop(lt_insert)) {
          ls_token.set(unique301);
